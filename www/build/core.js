(function () {
		  ...
		  createClass(ArraySeq, IndexedSeq);
		    ...
		    ArraySeq.prototype.__iterate = function(fn, reverse) {
		      ...
		      for (var ii = 0; ii <= maxIndex; ii++) {
		        if (fn(array[reverse ? maxIndex - ii : ii], ii, this$1) === false) {
		          return ii + 1;
		        }
		      }
		      return ii;
		  ...
		  createClass(List, IndexedCollection);
		    ...
		    List.prototype.__iterate = function(fn, reverse) {
		      ...
		      while ((value = values()) !== DONE) {
		        if (fn(value, index++, this$1) === false) {
		          break;
		        }
		      }
		      return index;
		  ...
		  createClass(Repeat, IndexedSeq);
		    ...
		    Repeat.prototype.__iterate = function(fn, reverse) {
		      ...
		      for (var ii = 0; ii < this.size; ii++) {
		        if (fn(this$1._value, ii, this$1) === false) {
		          return ii + 1;
		        }
		      }
		      return ii;
	...
	var nuclearJS = interopDefault(nuclear);
	...
	var keyMirror = interopDefault(index);
	...
	var Store = nuclearJS.Store;
	var toImmutable = nuclearJS.toImmutable;
	...
	function startValidate(state, ref) {
	...
	function validateFail(state, ref) {
	  var errorMessage = ref.errorMessage;

	  return state.withMutations(function (mState) { return mState
	...
	var Store$1 = nuclearJS.Store;
	var toImmutable$1 = nuclearJS.toImmutable;

	var INSTANCE$1 = new Store$1({
	  getInitialState: function getInitialState() {
	...
	function validateSuccess$1(state, ref) {
	  var authToken = ref.authToken;
	  var host = ref.host;

	  return toImmutable$1({ authToken: authToken, host: host });
	}
	...
	var Store$2 = nuclearJS.Store;

	var INSTANCE$2 = new Store$2({
	  getInitialState: function getInitialState() {
	    return true;
	  },
	...
	function storeRememberAuth(state, ref) {
	  var rememberAuth = ref.rememberAuth;
	...
	var Store$3 = nuclearJS.Store;
	var toImmutable$2 = nuclearJS.toImmutable;

	var INSTANCE$3 = new Store$3({
	  getInitialState: function getInitialState() {
	...
	var Store$4 = nuclearJS.Store;

	var INSTANCE$4 = new Store$4({
	  getInitialState: function getInitialState() {
	    return true;
	  },

	  initialize: function initialize() {
	    this.on(actionTypes$2.API_FETCH_ALL_START, function () { return true; });
	    this.on(actionTypes$2.API_FETCH_ALL_SUCCESS, function () { return false; });
	    this.on(actionTypes$2.API_FETCH_ALL_FAIL, function () { return false; });
	    this.on(actionTypes$2.LOG_OUT, function () { return false; });
	  },
	});

	var Store$5 = nuclearJS.Store;

	var INSTANCE$5 = new Store$5({
	  getInitialState: function getInitialState() {
	    return false;
	  },

	  initialize: function initialize() {
	    this.on(actionTypes$2.SYNC_SCHEDULED, function () { return true; });
	    this.on(actionTypes$2.SYNC_SCHEDULE_CANCELLED, function () { return false; });
	    this.on(actionTypes$2.LOG_OUT, function () { return false; });
	  },
	});
	...
	var Store$6 = nuclearJS.Store;
	var toImmutable$4 = nuclearJS.toImmutable;

	var INSTANCE$6 = new Store$6({
	  getInitialState: function getInitialState() {
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(restApiActionTypes.LOG_OUT, function () { return this$1.getInitialState(); });
	    /* eslint-enable no-use-before-define */
	  },
	});
	...
	function loadData(state, ref) {
	  ...
	  var newState = params.replace ? toImmutable$4({}) : state.get(entity);
	  var data = Array.isArray(result) ? result : [result];
	  var fromJSON = model.fromJSON || toImmutable$4;

	  return state.set(entity, newState.withMutations(function (mState) {
	    for (var i = 0; i < data.length; i++) {
	      var entry = fromJSON(data[i]);
	...
	var objectAssign = interopDefault(index$1);
	...
	function createApiActions$1(model) {
	  ...
	  apiActions.removeData = function removeData(reactor, id) {
	    onDeleteSuccess(reactor, model, { id: id });
	  };
	...
	var toImmutable$3 = nuclearJS.toImmutable;

	var createApiActions = createApiActions$1;
	...
	function createHasDataGetter(model) {
	  return [
	    ...
	    function (entityMap) { return !!entityMap; },
	  ];
	}
	...
	function createEntityMapGetter(model) {
	  return [
	    ['restApiCache', model.entity],
	    function (entityMap) { return entityMap || toImmutable$3({}); },
	  ];
	}
	...
	var hasMediaStates = ['playing', 'paused', 'unknown'];

	var MediaPlayer = function MediaPlayer(hass, stateObj) {
	  this.serviceActions = hass.serviceActions;
	  this.stateObj = stateObj;
	};

	var prototypeAccessors = { isOff: {},isIdle: {},isMuted: {},isPaused: {},isPlaying: {},isMusic: {},isTVShow: {},hasMediaControl: {},volumeSliderValue: {},supportsPause: {},supportsVolumeSet: {},supportsVolumeMute: {},supportsPreviousTrack: {},supportsNextTrack: {},supportsTurnOn: {},supportsTurnOff: {},supportsVolumeButtons: {},primaryText: {},secondaryText: {} };

	prototypeAccessors.isOff.get = function () {
	  return this.stateObj.state === 'off';
	};

	prototypeAccessors.isIdle.get = function () {
	...
	MediaPlayer.prototype.mediaPlayPause = function mediaPlayPause () {
	  this.callService('media_play_pause');
	};

	MediaPlayer.prototype.nextTrack = function nextTrack () {
	  this.callService('media_next_track');
	};

	MediaPlayer.prototype.playbackControl = function playbackControl () {
	  this.callService('media_play_pause');
	};

	MediaPlayer.prototype.previousTrack = function previousTrack () {
	  this.callService('media_previous_track');
	};

	MediaPlayer.prototype.setVolume = function setVolume (volume) {
	  this.callService('volume_set', { volume_level: volume });
	};

	MediaPlayer.prototype.togglePower = function togglePower () {
	  if (this.isOff) {
	    this.turnOn();
	  } else {
	    this.turnOff();
	...
	MediaPlayer.prototype.turnOff = function turnOff () {
	  this.callService('turn_off');
	};

	MediaPlayer.prototype.turnOn = function turnOn () {
	  this.callService('turn_on');
	};

	MediaPlayer.prototype.volumeDown = function volumeDown () {
	  this.callService('volume_down');
	};

	MediaPlayer.prototype.volumeMute = function volumeMute (mute) {
	...
	MediaPlayer.prototype.volumeUp = function volumeUp () {
	  this.callService('volume_down');
	};

	// helper method

	MediaPlayer.prototype.callService = function callService (service, data) {
	...
	function callApi$1(reactor, method, path, parameters) {
	  if ( parameters === void 0 ) parameters = null;

	  var authInfo = reactor.evaluate(authGetters.authInfo);
	  var url = (authInfo.host) + "/api/" + path;
	...
	var callApi = callApi$1;

	var Immutable = nuclearJS.Immutable;
	var toJS = nuclearJS.toJS;
	var ENTITY = 'entity';

	var ImmutableEntity = new Immutable.Record({
	  entityId: null,
	  domain: null,
	  objectId: null,
	  state: null,
	...
	var State = (function (ImmutableEntity) {
	  function State(entityId, state, lastChanged, lastUpdated, attributes) {
	    if ( attributes === void 0 ) attributes = {};

	    var ref = entityId.split('.');
	    var domain = ref[0];
	    var objectId = ref[1];
	  ...
	  if ( ImmutableEntity ) State.__proto__ = ImmutableEntity;
	  State.prototype = Object.create( ImmutableEntity && ImmutableEntity.prototype );
	  ...
	  State.prototype.domainModel = function domainModel (hass) {
	  ...
	  State.delete = function delete$1 (reactor, instance) {
	    return callApi(reactor, 'DELETE', ("states/" + (instance.entityId)));
	  };

	  State.save = function save (reactor, instance) {
	    var ref = toJS(instance);
	    var entityId = ref.entityId;
	    var state = ref.state;
	    var attributes = ref.attributes; if ( attributes === void 0 ) attributes = {};
	    var payload = { state: state, attributes: attributes };

	    return callApi(reactor, 'POST', ("states/" + entityId), payload);
	  };

	  State.fetch = function fetch (reactor, id) {
	    return callApi(reactor, 'GET', ("states/" + id));
	  };

	  State.fetchAll = function fetchAll (reactor) {
	    return callApi(reactor, 'GET', 'states');
	  };

	  State.fromJSON = function fromJSON (ref) {
	    var entity_id = ref.entity_id;
	    var state = ref.state;
	    var last_changed = ref.last_changed;
	    var last_updated = ref.last_updated;
	...
	var entityApiActions = createApiActions(State);

	var hasData = createHasDataGetter(State);

	var entityMap = createEntityMapGetter(State);

	var byId = createByIdGetter(State);

	var visibleEntityMap = [
	  entityMap,
	  function (entities) { return entities.filter(function (entity) { return !entity.attributes.hidden; }); },
	];
	...
	var entityActions = entityApiActions;
	var entityGetters = _getters$1;
	...
	var Store$7 = nuclearJS.Store;
	var Immutable$1 = nuclearJS.Immutable;

	var INSTANCE$7 = new Store$7({
	  getInitialState: function getInitialState() {
	...
	function notificationCreated(state, ref) {
	...
	function createNotification(reactor, message) {
	  reactor.dispatch(actionTypes$3.NOTIFICATION_CREATED, { message: message });
	}
	...
	var notificationActions = _actions$3;
	var getters$3 = _getters$2;
	...
	var Immutable$2 = nuclearJS.Immutable;
	var ENTITY$1 = 'event';

	var ImmutableEvent = new Immutable$2.Record({
	  event: null,
	  listenerCount: 0,
	}, 'Event');

	var Event = (function (ImmutableEvent) {
	  function Event(event, listenerCount) {
	    if ( listenerCount === void 0 ) listenerCount = 0;

	    ImmutableEvent.call(this, { event: event, listenerCount: listenerCount });
	  }

	  if ( ImmutableEvent ) Event.__proto__ = ImmutableEvent;
	  Event.prototype = Object.create( ImmutableEvent && ImmutableEvent.prototype );
	  ...
	  Event.fetchAll = function fetchAll (reactor) {
	    return callApi(reactor, 'GET', 'events');
	  };

	  Event.fromJSON = function fromJSON (ref) {
	    var event = ref.event;
	    var listener_count = ref.listener_count;
	  ...
	  Object.defineProperties( Event.prototype, prototypeAccessors );
	...
	var eventApiActions = createApiActions(Event);
	...
	var hasData$1 = createHasDataGetter(Event);

	var entityMap$1 = createEntityMapGetter(Event);

	var byId$1 = createByIdGetter(Event);
	...
	var actions$3 = eventApiActions;
	var getters$2 = _getters$3;
	...
	var Immutable$3 = nuclearJS.Immutable;
	var toImmutable$5 = nuclearJS.toImmutable;
	var ENTITY$2 = 'service';

	var ImmutableService = new Immutable$3.Record({
	  domain: null,
	  services: [],
	}, 'ServiceDomain');

	var ServiceDomain = (function (ImmutableService) {
	  function ServiceDomain(domain, services) {
	    ImmutableService.call(this, { domain: domain, services: services });
	  }

	  if ( ImmutableService ) ServiceDomain.__proto__ = ImmutableService;
	  ServiceDomain.prototype = Object.create( ImmutableService && ImmutableService.prototype );
	  ServiceDomain.prototype.constructor = ServiceDomain;
	  ...
	  ServiceDomain.fetchAll = function fetchAll () {
	    return callApi('GET', 'services');
	  };

	  ServiceDomain.fromJSON = function fromJSON (ref) {
	  ...
	  Object.defineProperties( ServiceDomain.prototype, prototypeAccessors );
	...
	function canToggleDomain(domain, servicesMap) {
	  ...
	  var serviceDomain = servicesMap.get(domain);
	...
	var hasData$2 = createHasDataGetter(ServiceDomain);

	var entityMap$2 = createEntityMapGetter(ServiceDomain);

	var byDomain = createByIdGetter(ServiceDomain);

	function hasService(domain, service) {
	  return [
	    byDomain(domain),
	    function (serviceDomain) { return !!serviceDomain && serviceDomain.services.has(service); },
	  ];
	}
	...
	var serviceApiActions = createApiActions(ServiceDomain);

	serviceApiActions.serviceRegistered = function serviceRegistered(reactor, domain, service) {
	  var serviceDomain = reactor.evaluateToJS(byDomain(domain));
	...
	var serviceActions = serviceApiActions;
	var getters$4 = _getters$4;
	...
	var isDataLoaded = [
	  ...
	  getters$2.hasData,
	  getters$4.hasData,
	  function (hasEntityData, hasEventData, hasServiceData) { return hasEntityData && hasEventData && hasServiceData; },
	];
	...
	var Store$8 = nuclearJS.Store;
	var toImmutable$6 = nuclearJS.toImmutable;

	var INSTANCE$8 = new Store$8({
	  getInitialState: function getInitialState() {
	...
	function componentLoaded(state, ref) {
	  var component = ref.component;

	  return state.push(component);
	}

	function serverConfigLoaded(state, ref) {
	...
	var Store$9 = nuclearJS.Store;
	var toImmutable$7 = nuclearJS.toImmutable;

	var INSTANCE$9 = new Store$9({
	  getInitialState: function getInitialState() {
	...
	function serverConfigLoaded$1(state, ref) {
	  var latitude = ref.latitude;
	  var longitude = ref.longitude;
	  var location_name = ref.location_name;
	  var temperature_unit = ref.temperature_unit;
	  var time_zone = ref.time_zone;
	...
	function fetchAll$1(reactor) {
	  callApi(reactor, 'GET', 'config').then(
	    function (config) { return configLoaded(reactor, config); });
	}

	function componentLoaded$1(reactor, component) {
	  reactor.dispatch(actionTypes$4.COMPONENT_LOADED, { component: component });
	}
	...
	var locationGPS = [
	  ...
	  function (latitude, longitude) { return ({ latitude: latitude, longitude: longitude }); },
	];
	...
	function isComponentLoaded(component) {
	  return [
	    ['serverComponent'],
	    function (components) { return components.contains(component); },
	  ];
	}
	...
	var configActions = _actions$4;
	var getters$5 = _getters$5;
	...
	var Store$10 = nuclearJS.Store;

	var INSTANCE$10 = new Store$10({
	  getInitialState: function getInitialState() {
	    return 'states';
	  },
	...
	var Store$11 = nuclearJS.Store;
	var toImmutable$8 = nuclearJS.toImmutable;

	var INSTANCE$11 = new Store$11({
	  getInitialState: function getInitialState() {
	  ...
	  initialize: function initialize() {
	    /* eslint-disable no-use-before-define */
	    this.on(actionTypes$5.PANELS_LOADED, panelsLoaded);
	    this.on(actionTypes$5.LOG_OUT, logOut$7);
	    /* eslint-enable no-use-before-define */
	  },
	});

	function panelsLoaded(state, ref) {
	  var panels = ref.panels;

	  return toImmutable$8(panels);
	}
	...
	var Store$12 = nuclearJS.Store;

	var INSTANCE$12 = new Store$12({
	  getInitialState: function getInitialState() {
	    return false;
	  },
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(actionTypes$5.LOG_OUT, logOut$8);
	    /* eslint-enable no-use-before-define */
	  },
	});

	function showSidebar(state, ref) {
	  var show = ref.show;
	...
	function showSidebar$1(reactor, show) {
	  reactor.dispatch(actionTypes$5.SHOW_SIDEBAR, { show: show });
	}

	function navigate$1(reactor, pane) {
	  reactor.dispatch(actionTypes$5.NAVIGATE, { pane: pane });
	}

	function panelsLoaded$1(reactor, panels) {
	  reactor.dispatch(actionTypes$5.PANELS_LOADED, { panels: panels });
	}
	...
	var activePanel = [
	  panels,
	  activePanelName,
	  function (pnls, activePane) { return pnls.get(activePane) || null; },
	];
	...
	var Store$13 = nuclearJS.Store;

	var INSTANCE$13 = new Store$13({
	  getInitialState: function getInitialState() {
	    return null;
	  },
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(actionTypes$6.LOG_OUT, logOut$9);
	    /* eslint-enable no-use-before-define */
	  },
	});

	function selectEntity(state, ref) {
	  var entityId = ref.entityId;
	...
	function selectEntity$1(reactor, entityId) {
	  reactor.dispatch(actionTypes$6.SELECT_ENTITY, { entityId: entityId });
	}
	...
	var Store$14 = nuclearJS.Store;

	var INSTANCE$14 = new Store$14({
	  getInitialState: function getInitialState() {
	...
	function dateSelected(state, ref) {
	  var date = ref.date;
	...
	var Store$15 = nuclearJS.Store;
	var toImmutable$9 = nuclearJS.toImmutable;

	var INSTANCE$15 = new Store$15({
	  getInitialState: function getInitialState() {
	...
	function entriesLoaded(state, ref) {
	  var date = ref.date;
	  var stateHistory = ref.stateHistory;

	  // set an empty map to indicate that data was loaded
	  if (stateHistory.length === 0) {
	    return state.set(date, toImmutable$9({}));
	  }

	  return state.withMutations(function (mState) {
	    stateHistory.forEach(
	      function (history) { return mState.setIn(
	...
	function logOut$11() {
	  return INSTANCE$15.getInitialState();
	}

	var Store$16 = nuclearJS.Store;

	var INSTANCE$16 = new Store$16({
	  getInitialState: function getInitialState() {
	    return false;
	  },
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(actionTypes$7.LOG_OUT, function () { return false; });
	  },
	});

	var Store$17 = nuclearJS.Store;
	var toImmutable$10 = nuclearJS.toImmutable;

	var INSTANCE$17 = new Store$17({
	  getInitialState: function getInitialState() {
	...
	function entriesLoaded$1(state, ref) {
	  var stateHistory = ref.stateHistory;

	  return state.withMutations(function (mState) {
	    stateHistory.forEach(
	      function (history) { return mState.set(
	        history[0].entity_id,
	        toImmutable$10(history.map(State.fromJSON))
	      ); }
	    );
	  });
	}
	...
	var Store$18 = nuclearJS.Store;
	var toImmutable$11 = nuclearJS.toImmutable;
	var ALL_ENTRY_FETCH = 'ALL_ENTRY_FETCH';

	var INSTANCE$18 = new Store$18({
	  getInitialState: function getInitialState() {
	    return toImmutable$11({});
	  },
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(actionTypes$7.RECENT_ENTITY_HISTORY_FETCH_SUCCESS,
	            entriesLoaded$2);
	    this.on(actionTypes$7.LOG_OUT, logOut$13);
	    /* eslint-enable no-use-before-define */
	  },
	});

	function entriesLoaded$2(state, ref) {
	  var stateHistory = ref.stateHistory;

	  var now = (new Date()).getTime();

	  return state.withMutations(function (mState) {
	    stateHistory.forEach(function (history) { return mState.set(history[0].entity_id, now); });
	...
	function logOut$13() {
	  return INSTANCE$18.getInitialState();
	}

	var toImmutable$12 = nuclearJS.toImmutable;

	var isLoadingEntityHistory = [
	...
	var entityHistoryMap = [
	  'entityHistory',
	];

	var entityHistoryForCurrentDate = [
	  currentDate,
	  entityHistoryMap,
	  function (date, map) { return map.get(date) || toImmutable$12({}); },
	];

	var hasDataForCurrentDate = [
	  currentDate,
	  entityHistoryMap,
	  function (date, map) { return !!map.get(date); },
	];

	var recentEntityHistoryMap = [
	  'recentEntityHistory',
	];

	var recentEntityHistoryUpdatedMap = [
	  'recentEntityHistory',
	];
	...
	function changeCurrentDate(reactor, date) {
	  reactor.dispatch(actionTypes$7.ENTITY_HISTORY_DATE_SELECTED, { date: date });
	}

	function fetchRecent(reactor, entityId) {
	  ...
	  return callApi(reactor, 'GET', url).then(
	    function (stateHistory) { return reactor.dispatch(
	      actionTypes$7.RECENT_ENTITY_HISTORY_FETCH_SUCCESS, { stateHistory: stateHistory }); },

	    function () { return reactor.dispatch(actionTypes$7.RECENT_ENTITY_HISTORY_FETCH_ERROR, {}); }
	  );
	}

	function fetchDate(reactor, date) {
	  reactor.dispatch(actionTypes$7.ENTITY_HISTORY_FETCH_START, { date: date });

	  return callApi(reactor, 'GET', ("history/period/" + date)).then(
	    function (stateHistory) { return reactor.dispatch(
	      actionTypes$7.ENTITY_HISTORY_FETCH_SUCCESS, { date: date, stateHistory: stateHistory }); },

	    function () { return reactor.dispatch(actionTypes$7.ENTITY_HISTORY_FETCH_ERROR, {}); }
	  );
	}

	function fetchSelectedDate(reactor) {
	  var date = reactor.evaluate(currentDate);
	...
	function register$8(reactor) {
	  reactor.registerStores({
	    currentEntityHistoryDate: INSTANCE$14,
	    entityHistory: INSTANCE$15,
	    isLoadingEntityHistory: INSTANCE$16,
	    recentEntityHistory: INSTANCE$17,
	    recentEntityHistoryUpdated: INSTANCE$18,
	  });
	}

	var actions$6 = _actions$7;
	var getters$8 = _getters$8;
	...
	var hasCurrentEntityId = [
	  currentEntityId,
	  function (entityId) { return entityId !== null; },
	];

	var currentEntity = [
	  currentEntityId,
	  entityGetters.entityMap,
	  function (entityId, entityMap) { return entityMap.get(entityId) || null; },
	];

	var currentEntityHistory = [
	  currentEntityId,
	  getters$8.recentEntityHistoryMap,
	  function (entityId, map) { return map.get(entityId); },
	];

	var isCurrentEntityHistoryStale = [
	  currentEntityId,
	  getters$8.recentEntityHistoryUpdatedMap,
	  function (entityId, map) { return isStaleTime(map.get(entityId)); },
	];
	...
	function register$7(reactor) {
	  reactor.registerStores({ moreInfoEntityId: INSTANCE$13 });
	}

	var actions$5 = _actions$6;
	var getters$7 = _getters$7;
	...
	var Store$19 = nuclearJS.Store;

	var INSTANCE$19 = new Store$19({
	  getInitialState: function getInitialState() {
	    return null;
	  },
	  ...
	  initialize: function initialize() {
	    /* eslint-disable no-use-before-define */
	    this.on(actionTypes$8.SELECT_VIEW, function (state, ref) {
	      var view = ref.view;
	...
	function validateView(state, ref) {
	  var model = ref.model;
	  var result = ref.result;
	  var params = ref.params;
	...
	function selectView(reactor, view) {
	  reactor.dispatch(actionTypes$8.SELECT_VIEW, { view: view });
	}
	...
	var Immutable$4 = nuclearJS.Immutable;
	...
	var views = [
	  entityGetters.entityMap,
	  function (entities) { return entities.filter(function (entity) { return entity.domain === 'group' &&
	                                        entity.attributes.view &&
	                                        entity.entityId !== DEFAULT_VIEW_ENTITY_ID; }); },
	];

	function addToMap(map, entities, groupEntity, recurse) {
	  if ( recurse === void 0 ) recurse = true;

	  groupEntity.attributes.entity_id.forEach(function (entityId) {
	...
	var currentViewEntities = [
	  ...
	  function (entities, view) {
	    ...
	    if (!viewEntity) {
	      return entities.filter(function (entity) { return !entity.attributes.hidden; });
	    }

	    return (new Immutable$4.Map()).withMutations(function (map) {
	...
	function register$9(reactor) {
	  reactor.registerStores({ currentView: INSTANCE$19 });
	}

	var actions$7 = _actions$8;
	var getters$9 = _getters$9;
	...
	var IS_SUPPORTED = history.pushState && !false;
	...
	function startSync(reactor) {
	  ...
	  var sync = {
	    ...
	    unwatchNavigationObserver: reactor.observe(activePanelName, function (pane) {
	      if (pane !== history.state.pane) {
	        history.pushState(pageState(pane, history.state.view), PAGE_TITLE,
	                          pageUrl(pane, history.state.view));
	      }
	    }),
	    unwatchViewObserver: reactor.observe(getters$9.currentView, function (view) {
	    ...
	    unwatchMoreInfoObserver: reactor.observe(
	      ...
	      function (moreInfoEntitySelected) {
	        if (moreInfoEntitySelected) {
	          ...
	          setTimeout(function () { return history.back(); }, 0);
	        }
	      }
	    ),
	  };
	...
	function register$6(reactor) {
	  reactor.registerStores({
	    currentPanel: INSTANCE$10,
	    panels: INSTANCE$11,
	    showSidebar: INSTANCE$12,
	  });
	}

	var actions$4 = _actions$5;
	var getters$6 = _getters$6;
	var urlSync = _urlSync;
	...
	function unscheduleSync(reactor) {
	  var sync = SCHEDULED_SYNCS[reactor.hassId];
	...
	function fetchAll(reactor) {
	  ...
	  return callApi(reactor, 'GET', 'bootstrap').then(function (data) {
	    reactor.batch(function () {
	      ...
	      actions$4.panelsLoaded(reactor, data.panels);
	    ...
	    reactor.dispatch(actionTypes$2.API_FETCH_ALL_FAIL, { message: message });
	...
	function start$1(reactor, ref) {
	  if ( ref === void 0 ) ref = {};
	  var skipInitialSync = ref.skipInitialSync; if ( skipInitialSync === void 0 ) skipInitialSync = false;
	...
	var actions$2 = _actions$2;
	var getters$1 = _getters;
	...
	var RETRY_TIME = 3000;
	var STREAMS = {};
	var EVENTS = ['state_changed', 'component_loaded', 'service_registered'].join(',');

	function stopStream$1(reactor) {
	  var stream = STREAMS[reactor.hassId];
	...
	function start(reactor, ref) {
	  if ( ref === void 0 ) ref = {};
	  var syncOnInitialConnect = ref.syncOnInitialConnect; if ( syncOnInitialConnect === void 0 ) syncOnInitialConnect = true;
	  ...
	  var reconnect = debounce(start.bind(null, reactor), RETRY_TIME);
	  var scheduleHealthCheck = debounce(start.bind(null, reactor), MAX_INACTIVITY_TIME);
	  var authToken = reactor.evaluate(authGetters.authToken);
	  var source = new EventSource(("/api/stream?api_password=" + authToken + "&restrict=" + EVENTS));
	  var syncOnConnect = syncOnInitialConnect;
	...
	var actions$1 = _actions$1;
	var getters = _getters$10;
	...
	var DEFAULT_ERROR_MSG = 'Unexpected result from API';
	 ...
	 */
	function validate(reactor, authToken, ref) {
	  if ( ref === void 0 ) ref = {};
	  var useStreaming = ref.useStreaming; if ( useStreaming === void 0 ) useStreaming = reactor.evaluate(getters.isSupported);
	  var rememberAuth = ref.rememberAuth; if ( rememberAuth === void 0 ) rememberAuth = false;
	  var host = ref.host; if ( host === void 0 ) host = '';

	  if (false) {}

	  reactor.dispatch(actionTypes.VALIDATING_AUTH_TOKEN, { authToken: authToken, host: host });

	  actions$2.fetchAll(reactor).then(
	    function () {
	      reactor.dispatch(actionTypes.VALID_AUTH_TOKEN, { authToken: authToken, host: host, rememberAuth: rememberAuth });
	    ...
	    function (ref) {
	      if ( ref === void 0 ) ref = {};
	      var message = ref.message; if ( message === void 0 ) message = DEFAULT_ERROR_MSG;
	...
	var isInvalidAttempt = [
	  'authAttempt',
	  'isInvalid',
	];

	var attemptErrorMessage = [
	  'authAttempt',
	  'errorMessage',
	];
	...
	var attemptAuthInfo = [
	  ...
	  ['authAttempt', 'host'],
	  function (authToken, host) { return ({ authToken: authToken, host: host }); },
	];
	...
	var authToken = [
	  ...
	  function (isValidating_, attemptToken_, currentToken_) { return (isValidating_ ? attemptToken_ : currentToken_); },
	];
	...
	var authInfo = [
	  ...
	  attemptAuthInfo,
	  currentAuthInfo,
	  function (isValidating_, attemptAuthInfo_, currentAuthInfo_) { return (isValidating_ ? attemptAuthInfo_ : currentAuthInfo_); },
	];
	...
	var observe = {
	  authToken: {
	    getter: [
	      ...
	      function (authToken, rememberAuth) { return (rememberAuth ? authToken : null); },
	    ],
	    defaultValue: null,
	  },
	  useStreaming: {
	...
	var localStoragePreferences = preferences;

	var Reactor = nuclearJS.Reactor;
	var reactorCount = 0;
	...
	function exposeModules(target, reactor, modules) {
	  Object.keys(modules).forEach(function (name) {
	    ...
	    if ('actions' in module) {
	      ...
	      Object.getOwnPropertyNames(module.actions).forEach(function (actionKey) {
	...
	var toImmutable$13 = nuclearJS.toImmutable;

	function expandGroup(groupState, entityMap) {
	  return toImmutable$13(groupState.attributes.entity_id.map(
	    function (entityId) { return entityMap.get(entityId); }).filter(function (ent) { return !!ent; }));
	}
	...
	var Store$20 = nuclearJS.Store;

	var INSTANCE$20 = new Store$20({
	  getInitialState: function getInitialState() {
	    var today = new Date();
	...
	function dateSelected$1(state, ref) {
	  var date = ref.date;
	...
	function logOut$14() {
	  return INSTANCE$20.getInitialState();
	}

	var Store$21 = nuclearJS.Store;

	var INSTANCE$21 = new Store$21({
	  getInitialState: function getInitialState() {
	    return false;
	  },

	  initialize: function initialize() {
	    this.on(actionTypes$9.LOGBOOK_ENTRIES_FETCH_START, function () { return true; });
	    this.on(actionTypes$9.LOGBOOK_ENTRIES_FETCH_SUCCESS, function () { return false; });
	    this.on(actionTypes$9.LOGBOOK_ENTRIES_FETCH_ERROR, function () { return false; });
	    this.on(actionTypes$9.LOG_OUT, function () { return false; });
	  },
	});

	var Immutable$5 = nuclearJS.Immutable;

	var ImmutableLogbookEntry = new Immutable$5.Record({
	  when: null,
	  name: null,
	  message: null,
	  domain: null,
	...
	var LogbookEntry = (function (ImmutableLogbookEntry) {
	  function LogbookEntry(when, name, message, domain, entityId) {
	    ImmutableLogbookEntry.call(this, {
	      when: when,
	      name: name,
	      message: message,
	      domain: domain,
	  ...
	  if ( ImmutableLogbookEntry ) LogbookEntry.__proto__ = ImmutableLogbookEntry;
	  LogbookEntry.prototype = Object.create( ImmutableLogbookEntry && ImmutableLogbookEntry.prototype );
	  LogbookEntry.prototype.constructor = LogbookEntry;

	  LogbookEntry.fromJSON = function fromJSON (ref) {
	    var when = ref.when;
	    var name = ref.name;
	    var message = ref.message;
	    var domain = ref.domain;
	...
	var Store$22 = nuclearJS.Store;
	var toImmutable$14 = nuclearJS.toImmutable;

	var INSTANCE$22 = new Store$22({
	  getInitialState: function getInitialState() {
	...
	function entriesLoaded$3(state, ref) {
	  var date = ref.date;
	  var entries = ref.entries;

	  return state.set(date, toImmutable$14(entries.map(LogbookEntry.fromJSON)));
	}
	...
	var Store$23 = nuclearJS.Store;
	var toImmutable$15 = nuclearJS.toImmutable;

	var INSTANCE$23 = new Store$23({
	  getInitialState: function getInitialState() {
	    return toImmutable$15({});
	  },

	  initialize: function initialize() {
	    /* eslint-disable no-use-before-define */
	    this.on(actionTypes$9.LOGBOOK_ENTRIES_FETCH_SUCCESS, entriesLoaded$4);
	    this.on(actionTypes$9.LOG_OUT, logOut$16);
	    /* eslint-enable no-use-before-define */
	  },
	});

	function entriesLoaded$4(state, ref) {
	  var date = ref.date;
	...
	function logOut$16() {
	  return INSTANCE$23.getInitialState();
	}

	function changeCurrentDate$1(reactor, date) {
	  reactor.dispatch(actionTypes$9.LOGBOOK_DATE_SELECTED, { date: date });
	}

	function fetchDate$1(reactor, date) {
	  reactor.dispatch(actionTypes$9.LOGBOOK_ENTRIES_FETCH_START, { date: date });

	  callApi(reactor, 'GET', ("logbook/" + date)).then(
	    function (entries) { return reactor.dispatch(
	      actionTypes$9.LOGBOOK_ENTRIES_FETCH_SUCCESS, { date: date, entries: entries }); },

	    function () { return reactor.dispatch(actionTypes$9.LOGBOOK_ENTRIES_FETCH_ERROR, {}); }
	  );
	}
	...
	var toImmutable$16 = nuclearJS.toImmutable;
	...
	var isCurrentStale = [
	  ...
	  function (currentDate_, map) { return isStaleTime$1(map.get(currentDate_)); },
	];

	var currentEntries = [
	  currentDate$1,
	  ['logbookEntries'],
	  function (date, map) { return map.get(date) || toImmutable$16([]); },
	];

	var isLoadingEntries = [
	...
	function register$10(reactor) {
	  reactor.registerStores({
	    currentLogbookDate: INSTANCE$20,
	    isLoadingLogbookEntries: INSTANCE$21,
	    logbookEntries: INSTANCE$22,
	    logbookEntriesUpdated: INSTANCE$23,
	  });
	}

	var actions$9 = _actions$10;
	var getters$10 = _getters$12;
	...
	function render(reactor, template) {
	  return callApi(reactor, 'POST', 'template', { template: template });
	}
	...
	var Store$24 = nuclearJS.Store;

	var INSTANCE$24 = new Store$24({
	  getInitialState: function getInitialState() {
	...
	var Store$25 = nuclearJS.Store;
	var toImmutable$17 = nuclearJS.toImmutable;

	var INSTANCE$25 = new Store$25({
	  getInitialState: function getInitialState() {
	    return toImmutable$17({
	      isListening: false,
	  ...
	  initialize: function initialize() {
	    ...
	    this.on(actionTypes$10.LOG_OUT, logOut$17);
	    /* eslint-enable no-use-before-define */
	  },
	});
	...
	function voiceResult(state, ref) {
	  var interimTranscript = ref.interimTranscript;
	  var finalTranscript = ref.finalTranscript;

	  return state.withMutations(function (map) { return map
	...
	function voiceTransmitting(state, ref) {
	  var finalTranscript = ref.finalTranscript;
	...
	function voiceDone() {
	  return INSTANCE$25.getInitialState();
	}
	...
	function process(reactor) {
	  var recognition = getResult(reactor);

	  if (!recognition) return;

	  var text = recognition.finalTranscript || recognition.interimTranscript;

	  reactor.dispatch(actionTypes$10.VOICE_TRANSMITTING, { finalTranscript: text });

	  serviceActions.callService(reactor, 'conversation', 'process', { text: text }).then(
	    function () { reactor.dispatch(actionTypes$10.VOICE_DONE); },
	    function () { reactor.dispatch(actionTypes$10.VOICE_ERROR); }
	  );
	}

	function stop$2(reactor) {
	  var result = getResult(reactor);
	...
	function listen(reactor) {
	  var finishForReactor = finish.bind(null, reactor);
	  finishForReactor();
	  ...
	  recognition.onstart = function () { return reactor.dispatch(actionTypes$10.VOICE_START); };
	  recognition.onerror = function () { return reactor.dispatch(actionTypes$10.VOICE_ERROR); };
	...
	var isVoiceSupported = ['isVoiceSupported'];
	...
	var extraInterimTranscript = [
	  interimTranscript,
	  finalTranscript,
	  function (interimTranscript_, finalTranscript_) { return interimTranscript_.slice(finalTranscript_.length); },
	];
	...
	function register$11(reactor) {
	  reactor.registerStores({ currentVoiceCommand: INSTANCE$25, isVoiceSupported: INSTANCE$24 });
	}

	var actions$11 = _actions$12;
	var getters$11 = _getters$13;
	...
	var HomeAssistant = function HomeAssistant() {
